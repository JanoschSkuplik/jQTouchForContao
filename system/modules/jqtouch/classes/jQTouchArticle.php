	...
	public function initializeJQTouch($objArticle) {
		...
		if ($objArticle->jqt_toolbar) 
		{
			...
			if ($objArticle->jqt_toolbarBackBtn)
			{
				...
				$objToolbar->backHtml = $objArticle->jqt_toolbarBackBtnText ? $objArticle->jqt_toolbarBackBtnText : 'Zurück';
			...
			// disable cache for selected pages
			if ($objArticle->jqt_nocache)
			{
				$arrCssID = deserialize($objArticle->cssID);
				$arrCssID[0] = $objArticle->alias . time();
